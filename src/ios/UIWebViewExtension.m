...
static char * hackishFixClassName = Nil;
static Class hackishFixClass = Nil;

- (UIView *)hackishlyFoundBrowserView {
    UIScrollView *scrollView = self.scrollView;
    hackishFixClassName = [[@[@"UI",@"Web",@"Browser",@"View",@"Minus",@"Accessory",@"View"] componentsJoinedByString:@""] UTF8String];
    NSString* UIClassString = [@[@"UI", @"Web", @"Browser", @"View"] componentsJoinedByString:@""];
    UIView *browserView = nil;
    for (UIView *subview in scrollView.subviews) {
        if ([NSStringFromClass([subview class]) hasPrefix:UIClassString]) {
            browserView = subview;
            break;
        }
    }
